...
module.exports.run = async function ({ api, event, args, Users }) {
    ...
    if (query == 'الكل') {
        let count = 1;
        msg += '=== قائمة تفاعل الكل===';
        for (const user of storage) {
            msg += `\n${count++}. ${user.name} - ${user.count}`;
        }
    } else if (query == 'الرانك') {
        ...
        msg += `${userID == senderID ? '🛡️ ترتيبك' : storage[rankUser].name} في المجموعة:「${rankUser + 1}」\n📨 عدد رسائلك:「${storage[rankUser].count}」\n🏅 الرانك:「${getRankName(storage[rankUser].count)}」`;
    }
