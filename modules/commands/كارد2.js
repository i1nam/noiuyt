...
module.exports.handleReply = async function({ api, event, handleReply }) {
    ...
    switch(handleReply.step) {
        case 1: {
            if(isNaN(event.body)) return api.sendMessage('يجب عليك كتابة رقم', event.threadID, event.messageID)   
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(`خترت الشخصية رقم ${event.body} \n\n رد على هاذة الرسالة بأسمك`, event.threadID, (err, info) => {
        ...
        case 2: {
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(`اخترت اسم ${event.body} \n\n رد على هاذة الرسالة بأسمك الثاني`, event.threadID, (err, info) => {
                global.client.handleReply.push({
                    step: 3,
                    name: this.config.name,
                    author: event.senderID,
...
module.exports.run = async function({ api, event }) {
    ...
    return api.sendMessage("رد على هاذه الرساله برقم التصميم (الارقام من 1 الى 800)", event.threadID, (err, info) => {
        global.client.handleReply.push({
            step: 1,
            name: this.config.name,
            author: event.senderID,
