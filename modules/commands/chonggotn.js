  ...
  if (senderID == global.data.botID) return;
    ...
    if(event.type == "message_unsend") {
     ...
     let name = await Users.getNameUser(senderID);
      if(getMsg.attachment[0] == undefined) return api.sendMessage(`[⚜️] لقد تم حذف رسالة من قبل ${name} \n\nالرسالة التي حذف: ${getMsg.msgBody}`,threadID)
      else {
            let num = 0
            let msg = {
              body:`[⚜️] تم حذف مرفق بواسطة ${name}    \n${getMsg.attachment.length} المرفق الذي حذف${(getMsg.msgBody != "") ? `\n\nالرسالة المحذوفة: ${getMsg.msgBody}` : ""}`,
              attachment:[],
...
module.exports.run = async function({ api, event, Threads }) {
	...
	return api.sendMessage(`  ${(data["resend"] == true) ? "تم تشغيله" : "تم ايقافه"} بنجاح!`, threadID, messageID);
}